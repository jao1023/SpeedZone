...
// NOTA: Se você está usando $conn no seu connection.php, mude $conn para $conn aqui.
...
// Assume-se que a variável de conexão é $conn conforme o último código enviado
$conn = $conn; 

if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
    // Captura e sanitiza o termo de pesquisa
    $searchTerm = $conn->real_escape_string(trim($_GET['search']));
    
    // Constrói a cláusula WHERE para pesquisar no nome ou email
    $whereClause = " WHERE primeiro_nome LIKE '%$searchTerm%' OR email LIKE '%$searchTerm%' ";
}

// 2. CONSULTA SQL DINÂMICA
...
<head>
    ...
    <style>
        ...
        /* 3. ESTILOS DA BARRA DE PESQUISA (Lupa integrada) */
                    ...
                    <div class="data-table">
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) { 
                                ...
                                // Lógica de classes baseada nos dados do BD
